...
export default function VideosCarousel({ videos, onVideoSelect }: VideosCarouselProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-4 overflow-x-auto pb-2 [scrollbar-width:thin] [scrollbar-color:transparent_transparent] hover:[scrollbar-color:hsl(var(--muted))_transparent]">
        {youtubeVideos.map((video) => (
          <button
            key={video.id}
            onClick={() => onVideoSelect(video)}
            className={cn(
              "group relative flex-shrink-0 w-[280px] h-[158px] rounded-lg overflow-hidden",
