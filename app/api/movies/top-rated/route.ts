...
import { getTopRatedMovies, TMDBResponse, TMDBMovie } from "@/lib/tmdb";
...
export async function GET(request: NextRequest): Promise<NextResponse<TMDBResponse<TMDBMovie> | { error: string }>> {
  try {
    ...
    const moviesPromise = getTopRatedMovies(page);
    ...
    try {
      ...
      console.warn(`Top rated movies ${isTimeout ? 'timeout' : 'error'}, returning empty results:`, error);
      return NextResponse.json(
        {
          page: 1,
          results: [],
