...
import { getPopularMovies, TMDBResponse, TMDBMovie } from "@/lib/tmdb";
...
export async function GET(request: NextRequest): Promise<NextResponse<TMDBResponse<TMDBMovie> | { error: string }>> {
  try {
    ...
    const moviesPromise = getPopularMovies(page);
    ...
    try {
      ...
      console.warn(`Popular movies ${isTimeout ? 'timeout' : 'error'}, returning empty results:`, error);
      return NextResponse.json(
        {
          page: 1,
          results: [],
