...
export async function POST(
  ...
  try {
    ...
    // Verify comment exists and belongs to the list
    ...
    const existingReaction = await db.listCommentReaction.findUnique({
      where: {
    ...
    // Create reaction
    const reaction = await db.listCommentReaction.create({
      data: {
        commentId,
        userId: user.id,
        reactionType,
    ...
    await triggerListCommentsUpdated(listId, { commentId, action: "added", reactionType });
...
export async function DELETE(
  ...
  try {
    ...
    const reaction = await db.listCommentReaction.findUnique({
      where: {
        commentId_userId_reactionType: {
          commentId,
          userId: user.id,
    ...
    await db.listCommentReaction.delete({
    ...
    await triggerListCommentsUpdated(listId, { commentId, action: "removed", reactionType });
...
export async function GET(
  ...
  try {
    ...
    const reactions = await db.listCommentReaction.findMany({
      where: { commentId },
      include: {
        user: {
          select: {
