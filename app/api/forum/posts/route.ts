    ...
    if (user.isSuspended) {
      ...
      if (user.suspendedUntil && new Date(user.suspendedUntil) <= new Date()) {
        ...
        if (user.suspendedUntil) {
          return NextResponse.json(
            { error: `Your account is suspended until ${new Date(user.suspendedUntil).toLocaleDateString()}. You cannot create new posts during this time. Please contact support if you believe this is an error.` },
            { status: 403 }
          );
        } else {
          return NextResponse.json(
            { error: "Your account has been suspended. You cannot create new posts. Please contact support if you believe this is an error." },
            { status: 403 }
          );
        }
      }
