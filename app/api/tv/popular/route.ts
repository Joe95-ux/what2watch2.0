...
import { getPopularTV, TMDBResponse, TMDBSeries } from "@/lib/tmdb";
...
export async function GET(request: NextRequest): Promise<NextResponse<TMDBResponse<TMDBSeries> | { error: string }>> {
  try {
    ...
    const tvPromise = getPopularTV(page);
    ...
    try {
      ...
      console.warn(`Popular TV ${isTimeout ? 'timeout' : 'error'}, returning empty results:`, error);
      return NextResponse.json(
        {
          page: 1,
          results: [],
